...
const parseReplyMessages = (value) => {
  ...
  try {
    ...
    console.error("Reply message parsing error:", error.message);
    return [];
  }
};
  ...
  try {
    ...
    const finalUsers = await Promise.all(hosts.map(async (host) => {
      ...
      const enrichedProperty = await Promise.all(properties.map(async (property) => {
        ...
        const safety = property.safety_amenities
          ? property.safety_amenities
            .split(",")
            .map((id) => parseInt(id.trim()))
            .filter((id) => !isNaN(id))
          : [];

        const idealFor = property.ideal_for
          ? property.ideal_for
            .split(",")
            .map((id) => parseInt(id.trim()))
            .filter((id) => !isNaN(id))
        ...
        let house_rules = property.house_rules
...
export const updateBlogManagement = async (req, res) => {
  try {
    ...
    const image = req.files?.image || req.files || [];
    const userDetails = await getDataByLabel(DB_TABLE.admin, "admin_id", admin_id);
  ...
  try {
    ...
    if (userSupportQuery.length > 0) {
      await Promise.all(userSupportQuery.map(async (item) => {
        item.reply_message = parseReplyMessages(item.reply_message);
      }));
    }
...
export const supportTicketReply = async (req, res) => {
  try {
    ...
    if (supportTicket[0].reply_message == null) {
      ...
      messages = parseReplyMessages(supportTicket[0].reply_message);
...
export const updateCancellationPolicy = async (req, res) => {
  ...
  console.log("Received data for update:", req.query, req.body);
  if (!policy_id) {
...
const CANCELLATION_POLICY_SETTINGS_TYPE = 5;
const DEFAULT_CANCELLATION_POLICY_SETTINGS = {
  thirty_days: 5,
  ten_days: 3,
  seven_days: 10,
  same_day: 105,
...
const parseCancellationPolicySettings = (content) => {
  if (!content) {
    return { ...DEFAULT_CANCELLATION_POLICY_SETTINGS };
  }

  try {
    const parsed = JSON.parse(content);
    return {
      thirty_days: Number(parsed?.thirty_days ?? DEFAULT_CANCELLATION_POLICY_SETTINGS.thirty_days),
      ten_days: Number(parsed?.ten_days ?? DEFAULT_CANCELLATION_POLICY_SETTINGS.ten_days),
      seven_days: Number(parsed?.seven_days ?? DEFAULT_CANCELLATION_POLICY_SETTINGS.seven_days),
      same_day: Number(parsed?.same_day ?? DEFAULT_CANCELLATION_POLICY_SETTINGS.same_day),
    };
  } catch (error) {
    return { ...DEFAULT_CANCELLATION_POLICY_SETTINGS };
  }
...
export const getCancellationPolicySettings = async (req, res) => {
  try {
    const admin_id = req.user?.admin_id;
    const userDetails = await getDataByLabel(DB_TABLE.admin, "admin_id", admin_id);
    ...
    const settings = await getDataByLabel(
      DB_TABLE.content,
      "content_type",
      CANCELLATION_POLICY_SETTINGS_TYPE
    );

    if (!settings?.length) {
      await addDataIntoTable(DB_TABLE.content, {
        content_type: CANCELLATION_POLICY_SETTINGS_TYPE,
        content: JSON.stringify(DEFAULT_CANCELLATION_POLICY_SETTINGS),
      });
      ...
      return handleSuccess(
        ...
        StatusCode.status200,
        Message.dataFoundSuccessful,
        DEFAULT_CANCELLATION_POLICY_SETTINGS
      );
    }
    ...
    return handleSuccess(
      ...
      StatusCode.status200,
      Message.dataFoundSuccessful,
      parseCancellationPolicySettings(settings[0]?.content)
    );
  } catch (error) {
    console.error("Error :", error);
    return handleError(res, StatusCode.status500, Message.INTERNAL_SERVER_ERROR);
...
export const updateCancellationPolicySettings = async (req, res) => {
  try {
    const admin_id = req.user?.admin_id;
    const userDetails = await getDataByLabel(DB_TABLE.admin, "admin_id", admin_id);
    ...
    const payload = {
      thirty_days: Number(req.body?.thirty_days),
      ten_days: Number(req.body?.ten_days),
      seven_days: Number(req.body?.seven_days),
      same_day: Number(req.body?.same_day),
    };

    const existing = await getDataByLabel(
      DB_TABLE.content,
      "content_type",
      CANCELLATION_POLICY_SETTINGS_TYPE
    );
    ...
    return handleSuccess(
      res,
      StatusCode.status200,
      Message.cancellationPolicySettingsUpdated,
      payload
    );
  } catch (error) {
    console.error("Error :", error);
