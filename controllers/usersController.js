  ...
  try {
    ...
    if (bhk) {
      property = property.filter(
        (item) => Number(item.bedrooms) === Number(bhk)
      );
    }
    ...
    if (bed_bath) {
      property = property.filter(
        (item) =>
          Number(item.beds) === Number(bed_bath) ||
          Number(item.bathrooms) === Number(bed_bath)
      );
    }
    ...
    if (min_price && max_price) {
      property = property.filter((item) => {
        ...
        return (
          rent >= Number(min_price) &&
          rent <= Number(max_price)
        );
      });
    }
    ...
    if (optionalUserId) {
      ...
      const wishlistRows =
        await getWishlistPropertyIdsForUser(
          optionalUserId,
          propertyIds
        );

      wishlistSet = new Set(
        wishlistRows.map((r) => r.property_id)
      );
    }
    ...
    await Promise.all(
      property.map(async (item) => {
        ...
        let ideal_for = item.ideal_for
          ? item.ideal_for.split(",")
          : [];

        let house_rules = item.house_rules
          ? item.house_rules.split(",")
          : [];
        ...
        if (
          ...
          item.amenities_label =
            await getPropertyAmenitits(amenities);
        }

        if (
          Array.isArray(safety_amenities) &&
          safety_amenities.length > 0
        ) {
          item.safety_amenities_label =
            await getAllPropertySafety(
              safety_amenities
            );
        }

        if (
          Array.isArray(ideal_for) &&
          ideal_for.length > 0
        ) {
          item.ideal_for_label =
            await getPropertyIdealFor(ideal_for);
        }
        ...
        if (
          ...
          item.house_rules_label =
            await getHouseRulesByIds(
              house_rules
            );
        }

        [item.rating] = await getHostRating(
          item.host_id
        );

        let review = await getPropertyReview(
          item.property_id
        );
        ...
        item.is_wishlist = wishlistSet.has(
          item.property_id
        );

        [item.canellationPolicy] =
          await getDataByLabel(
        ...
        item.booked_dates = bookings.flatMap(
          (b) =>
            expandDates(
              b.start_date,
              b.end_date
        ...
        const offer =
          await getActiveOfferByPropertyId(
            item.property_id
          );
    ...
    await Promise.all(
      property.map(async (item) => {
        ...
        item.video_url = item?.video_url
          ? `${process.env.BASE_URL}profile/${item?.video_url}`
          : "";
        let amenities = item.amenities ? item.amenities.split(",").map(id => parseInt(id.trim())).filter(id => !isNaN(id)) : [];
        let safety_amenities = item.safety_amenities
          ? item.safety_amenities.split(",").map(id => parseInt(id.trim())).filter(id => !isNaN(id))
          : [];
        let ideal_for = item.ideal_for ? item.ideal_for.split(",").map(id => parseInt(id.trim())).filter(id => !isNaN(id)) : [];
        let house_rules = item.house_rules ? item.house_rules.split(",").map(id => parseInt(id.trim())).filter(id => !isNaN(id)) : [];
...
export const getPropertyByIdController = async (req, res) => {
  try {
    ...
    const optionalUserId = req.user?.id || null;
    ...
    if (optionalUserId) {
      ...
      isWishlist = wishlistRows.some(
        (item) => Number(item.property_id) === Number(property.property_id)
      );
    }
    ...
    const amenities = property.amenities ? property.amenities.split(",").map(id => parseInt(id.trim())).filter(id => !isNaN(id)) : [];
    const safety_amenities = property.safety_amenities
      ? property.safety_amenities.split(",").map(id => parseInt(id.trim())).filter(id => !isNaN(id))
      : [];
    const ideal_for = property.ideal_for ? property.ideal_for.split(",").map(id => parseInt(id.trim())).filter(id => !isNaN(id)) : [];
    const house_rules = property.house_rules ? property.house_rules.split(",").map(id => parseInt(id.trim())).filter(id => !isNaN(id)) : [];
...
export const propertyBooking = async (req, res) => {
  ...
  const propertyDetails = await getPropertyById(
    property_id,
    req.user?.id
  );
    ...
    plans?.forEach((item) => {
      if (typeof item?.content === "string") {
        try {
          ...
          item.content = Array.isArray(parsedContent)
            ? parsedContent
            : [];
  ...
  try {
    ...
    if (recentBookings.length > 0) {
      await Promise.all(recentBookings.map(async (item) => {
        ...
        if (item.vat_details) {
          item.vat_details = safeJsonParse(item.vat_details);
        }
  ...
  try {
    ...
    if (ticket.length > 0) {
      await Promise.all(ticket.map(async (item) => {
        item.reply_message = safeJsonParse(item.reply_message, []);
      }));
    }
...
const safeJsonParse = (value, fallback = null) => {
  try {
    ...
    try {
      ...
      console.error("JSON parsing error:", secondError.message);
      return fallback;
    }
  }
};
