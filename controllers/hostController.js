...
export const fetchProfile = async (req, res) => {
  try {
    ...
    let stripeSummary = {
      kyc_completed: false,
      isStripeKycVerified: false,
    };

    if (userDetails?.[0]?.stripe_account_id) {
      try {
        const stripeAccount = await stripe.accounts.retrieve(
          userDetails[0].stripe_account_id,
          {
        ...
        stripeSummary = {
          kyc_completed: isStripeKycCompleted(stripeAccount),
          isStripeKycVerified: isStripeIdentityVerified(stripeAccount),
        };
      } catch (stripeError) {
        console.log("Stripe fetch error :", stripeError.message);
      }
    }
    ...
    const finalData = userDetails?.map((item, i) => {
      return {
        ...
        kyc_completed: stripeSummary.kyc_completed,
        isStripeKycVerified: stripeSummary.isStripeKycVerified,
      };
    });
