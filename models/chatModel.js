...
// 🔴 Admin → Hosts
export async function getAllHosts(adminId) {
    ...
    try {
        ...
        const result = await db.query(
            ...
            INNER JOIN host_master h
                ON (
                    (
                        c.participant_one_id = h.host_id
                        AND c.participant_one_type IN ('host', 'hostBusiness')
                    )

                    OR

                    (
                        c.participant_two_id = h.host_id
                        AND c.participant_two_type IN ('host', 'hostBusiness')
                    )
                )
            ...
            WHERE
                (
                    (
                        ...
                        AND c.participant_one_type = 'admin'
                    )
                    ...
                    (
                        ...
                        AND c.participant_two_type = 'admin'
                    )
                )
        ...
        console.log("GET ALL HOST ERROR =>", error);
