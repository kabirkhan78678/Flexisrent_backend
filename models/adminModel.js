...
export const getManagePayoutListModel = async (report_id) => {
  const query = `
      SELECT 
        ...
        pm.payment_status,
        CASE
  WHEN pm.payment_status = 'REJECTED' THEN 'REJECTED'
  ELSE COALESCE(pm.payout_status, 'PENDING')
  ...
  const query = `
    SELECT
        ...
        SUM(
          CASE 
            ...
            THEN COALESCE(
              pm.host_payout_amount,
              ROUND(
                COALESCE(pm.total_amount, 0) - COALESCE(pm.admin_earnings, 0),
                2
              )
            )
            ELSE 0
      ...
      COALESCE(
        ...
        0
      ) AS failed_payout_count,

      COALESCE(
        SUM(
          CASE 
            WHEN pm.payment_status = 'REJECTED'
            THEN 1
            ELSE 0
          END
        ),
        0
      ) AS rejected_payout_count
